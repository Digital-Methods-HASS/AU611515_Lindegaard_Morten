...
kongerækker <- read.csv2("spreadsheet_for_kongerækken.csv")
```


```{r eval = TRUE}
kongerækker %>% 
  mutate(duration=Slutning_på_regeringstid - Start_på_regeringstid)

kongerækker %>% 
  mutate(midyear =Slutning_på_regeringstid - (Slutning_på_regeringstid - Start_på_regeringstid)/2)
...
```{r}
ggplot(data = kongeplotting, aes(x = midyear, y = duration))+
  geom_col(alpha = 0.5)+
  geom_smooth()+
  labs(title ="danish king duration of reign",
       x="year",
       y="duration")
...
The datavisualizations shows that murderrates have been declining in Europe since the medieval times.
However the drop in number of homicides was much steeper from 1400-1800 than from 1800-today. If civilisation is purely measured from homiciderates one might therefore say that europe hasnt been significantly evolving in regards to civilisation since 1800's. 
